...
from lib.models import ChecklistQuestion, Objection, Pillar, Solution, Stage
...
def main() -> None:
    ...
    checklist_questions = load_content(content / "checklists", ChecklistQuestion)
    ...
    stage_questions = [item for item in checklist_questions
                       if item.stage == stage.id and item.pillar in {pillar.id for pillar in weak_pillars}]
    ...
    output_path.write_text(template.render(stage=stage, scores=scores, weak_pillars=weak_pillars,
                                           objections=stage_objections, solutions=relevant_solutions,
                                           checklist_questions=stage_questions), encoding="utf-8")

    print(f"Generated: {output_path}")
    print(f"Weak pillars: {', '.join(pillar.name for pillar in weak_pillars) or 'none'}")
    print(f"Objections: {len(stage_objections)} | Solutions: {len(relevant_solutions)} | Checklist questions: {len(stage_questions)}")
